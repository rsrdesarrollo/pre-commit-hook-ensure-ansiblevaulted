...
if ! OPT_ENCRYPTED_EXT=$(yq -r '.["encrypted-extension"] // "enc.yaml"' "${SELF_CFG}"); then
  echo "- ${_ERR}: config error while reading encrypted extension"
  exit 1
fi

OPT_VAULT_PASSWORD_FILE=$(yq -r '.["vault-password-file"] // ""' "${SELF_CFG}")
...
OPT_ENSURE_IGNORED=$(yq -r '.["track-git-ignored"] // ".with-error"' "${SELF_CFG}")
